...
static funcionario_set_contrato_t funcionario_set_contrato_callback[FUNCIONARIO_CONTRATO_LEN] = {
    &funcionario_set_clt,
    &funcionario_set_terceirizado};

static funcionario_calcula_pagamento_t funcionario_calcula_pagamento_callback[FUNCIONARIO_CONTRATO_LEN] = {
    &funcionario_calcula_pagamento_clt,
    &funcionario_calcula_pagamento_terceirizado};

// -------------------------------------------------------
// Implementacao dos metodos publicos
// -------------------------------------------------------
funcionario_t *funcionario_init_dinamico(funcionario_contrato_typedef tipo_contrato, funcionario_tipo_contrato_t contrato, char *nome, uint32_t contratos_assinados, double comissao_por_contrato, double bonus_anual) // FEATURE PEDIDA -------
{
    ...
    if ((strlen((const char *)nome) + 1) <= FUNCIONARIO_LEN_NOME)
    {
        ...
        if (ret != NULL)
        {
            ...
            ret->bonus_anual = bonus_anual; // FEATURE PEDIDA -------
...
funcionario_retorno_typedef funcionario_init_estatico(funcionario_t *obj, funcionario_contrato_typedef tipo_contrato, funcionario_tipo_contrato_t contrato, char *nome, uint32_t contratos_assinados, double comissao_por_contrato, double bonus_anual) // FEATURE PEDIDA -------
{
    ...
    if ((strlen((const char *)obj->nome) + 1) <= FUNCIONARIO_LEN_NOME)
    {
        ...
        obj->bonus_anual = bonus_anual; // FEATURE PEDIDA -------
...
double funcionario_calcular_total_pagar_funcionario(funcionario_t *obj)
{
    ...
    if (funcionario_is_valid(obj))
        ret = (obj->comissao_por_contrato * obj->contratos_assinados) + funcionario_calcula_pagamento_callback[obj->tipo_contrato](obj) + obj->bonus_anual; // FEATURE PEDIDA -------
...
static funcionario_retorno_typedef funcionario_set_clt(funcionario_t *func_obj, funcionario_tipo_contrato_t *clt_obj)
{
    ...
    if (func_obj != NULL && clt_obj != NULL)
    {
        ...
        if (func_obj->tipo_contrato == FUNCIONARIO_CONTRATO_CLT && clt_obj->clt.salario >= FUNCIONARIO_MIN_SALARIO)
        {
            ...
            func_obj->contrato.clt.hora_extra = clt_obj->clt.hora_extra; // FEATURE PEDIDA -------
            ret = FUNCIONARIO_RETORNO_OK;
        }
    }
...
static double funcionario_calcula_pagamento_clt(funcionario_t *obj)
{
    ...
    if (obj != NULL)
    {
        ...
        ret += obj->contrato.clt.hora_extra; // FEATURE PEDIDA -------
    }
